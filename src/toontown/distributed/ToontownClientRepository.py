...
class ToontownClientRepository(OTPClientRepository.OTPClientRepository):
    ...
    def __init__(self, serverVersion):
        ...
        self.ttcyFriendsManager = self.generateGlobalObject(OtpDoGlobals.OTP_DO_ID_TTCY_FRIENDS_MANAGER, 'TTCYFriendsManager')

        self.argManager = self.generateGlobalObject(OtpDoGlobals.OTP_DO_ID_TTCY_ARG_MANAGER, 'ARGManager')
    ...
    def __sendGetAvatarDetails(self, avId, pet=0):
        if pet:
            self.ttcyFriendsManager.d_getPetDetails(avId)
        else:
            self.ttcyFriendsManager.d_getAvatarDetails(avId)
    ...
    def removeFriend(self, avatarId):
        self.ttcyFriendsManager.d_removeFriend(avatarId)
    ...
    def sendGetFriendsListRequest(self):
        ...
        self.friendsListError = 0
        self.ttcyFriendsManager.d_requestFriendsList()
    ...
    def requestAvatarInfo(self, avId):
        if avId == 0:
            return
        self.ttcyFriendsManager.d_requestAvatarInfo([avId])
    ...
    def sendAvatarInfoRequests(self, task = None):
        ...
        if len(self.avatarInfoRequests) == 0:
            return
        self.ttcyFriendsManager.d_requestAvatarInfo(self.avatarInfoRequests)
